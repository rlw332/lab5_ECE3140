...
static int cmp_time(const realtime_t *proc_one, const realtime_t *proc_two) {
...
static void add_sorted(process_t *proc, process_queue_t *queue) {
...
unsigned int *process_select(unsigned int *cursp) {
    if (cursp && current_process_p) {
        ...
        if (current_process_p->is_realtime) {
            realtime_t deadline_abs = {
                .sec = current_process_p->arrival_time->sec + current_process_p->deadline->sec,
                .msec = current_process_p->arrival_time->msec + current_process_p->deadline->msec
            };
            if (deadline_abs.msec >= 1000) {
                deadline_abs.sec += deadline_abs.msec / 1000;
                deadline_abs.msec %= 1000;
            }
